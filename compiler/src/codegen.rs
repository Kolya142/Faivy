...
impl CodeGen {
	...
	if let IRData::DeclareFunction(ref dt, ref name, ref typ, ref args, ref body) = ir.data {
		...
		if ispolymorph {
		    ...
		    self.functions.push(CodeGenFunction{name: name.to_string(), real_name: reg.to_string(), typ: *typ.clone(), args: args.clone(), decl: dt.clone(), ispolymorph: false, body: None, other_implementations: vec![]});
		}
	    }
